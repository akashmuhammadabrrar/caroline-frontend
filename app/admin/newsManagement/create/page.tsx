...
import { useCreateNewsArticleMutation, useGetNewsCategoriesQuery } from '@/redux/features/admin/adminNewsApi';
import { toast } from 'react-hot-toast';

export default function CreateArticlePage() {
  const router = useRouter();
  const [createArticle, { isLoading }] = useCreateNewsArticleMutation();
  ...
  const handleSubmit = async (status: 'DRAFT' | 'PUBLISHED') => {
    ...
    try {
      ...
      // Create a specific payload for the 'data' field
      const articlePayload = {
        ...formData,
        category: Number(formData.category), // Ensure it's a number
        status
      };

      // Append the data payload as a stringified JSON
      dataToSend.append('data', JSON.stringify(articlePayload));

      const response = await createArticle(dataToSend).unwrap();
      if (response.success) {
      ...
      <div className="sticky top-0 z-50 bg-[#0B0D2C]/80 backdrop-blur-md border-b border-gray-800/50">
        <div className="max-w-7xl mx-auto px-6 h-16 flex items-center justify-between">
          <div className="flex items-center gap-4">
            ...
            <h1 className="text-xl font-black tracking-tight">Create <span className="text-cyan-400">New Article</span></h1>
          </div>
        ...
        <div className="space-y-10">
            ...
            >
              {imagePreview ? (
                ...
                <div className="text-center p-12">
                  ...
                  <p className="text-lg font-bold text-white mb-2">Upload Article Banner</p>
                  <p className="text-sm text-gray-500">Recommended size: 1200 x 600 pixels (Max 5MB)</p>
                </div>
              )}
              <input
                type="file"
          ...
          <div className="bg-[#12142d] border border-gray-800 rounded-3xl p-6 flex flex-wrap items-center justify-between gap-6">
            <div className="flex items-center gap-4">
              ...
              <span className="text-xs font-bold text-gray-400">Title Added</span>

              <div className={`p-2 rounded-full ${formData.content.length > 50 ? 'bg-emerald-500/20 text-emerald-400' : 'bg-gray-800 text-gray-600'}`}>
                <CheckCircle2 size={16} />
              </div>
              <span className="text-xs font-bold text-gray-400">Content Ready</span>

              <div className={`p-2 rounded-full ${imagePreview ? 'bg-emerald-500/20 text-emerald-400' : 'bg-gray-800 text-gray-600'}`}>
                <CheckCircle2 size={16} />
              </div>
              <span className="text-xs font-bold text-gray-400">Banner Uploaded</span>
            </div>
